...
# better to negate (above says does != 1) bc could be null, but really only discontinued if = 1
...
# aggregate functions (min / max are examples) need to be in select statements
...
# inner join - only see rows in table where match occurs
